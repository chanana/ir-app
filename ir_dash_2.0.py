...
smiles = pd.read_csv("csv_data/smiles_fixed.csv", encoding="latin-1", header=0,
                     index_col=0)
...
def add_text_to_file(sample_name):
    ...
    if sample_name not in smiles.index:
        canvas_draw.text((0, 400), sample_name + ": Not Found", outline="black",
                         fill="black",
                         font=font)
    else:
        canvas_draw.text((20, 400),
                         sample_name + ": " + smiles.loc[sample_name, "Trivial name"],
                         outline="black", fill="black", font=font)
...
def smiles_to_grid_png(sample_name_list):
    smile_list = [smiles.loc[i, "SMILES"] if i in smiles.index else "CC" for i in
                  sample_name_list]
    mols = [Chem.MolFromSmiles(i) for i in smile_list]
    legends = [smiles.loc[i, "Trivial name"] if i in smiles.index else "None" for i in
               sample_name_list]
...
def json_to_sample_names_list(json_data):
    return [json_data["points"][i]["customdata"] for i in
            range(0, len(json_data["points"]))]
...
app.layout = dbc.Tabs([
    dbc.Tab(id='clustering-tab', label='Clustering', children=[
        dbc.Container([
            html.Div([
                # dataset selection + submit button
                dbc.Container([
                    dbc.Row([
                        dbc.Col([html.Label("Collection"),
                                 dcc.Dropdown(id='collection-choice', clearable=False,
                                              options=[{'label': i.upper(), 'value': i} for i in
                                                       ['az', 'mtp', 'mtp and az']],
                                              value='mtp and az'), ], width=2),
                        dbc.Col([html.Label("Region of Spectrum"),
                                 dcc.Dropdown(id='region-choice', clearable=False,
                                              options=[{'label': i.title(), 'value': i} for i in
                                                       ['full', 'fingerprint']],
                                              value='fingerprint'), ], width=2),
                        dbc.Col([html.Label("Scaling"),
                                 dcc.Dropdown(id='scaling-choice', clearable=False,
                                              options=[{'label': i.title(), 'value': i} for i in
                                                       ['scaled', 'normalized']],
                                              value='scaled'), ], width=2),
                        dbc.Col([html.Label("Visualization"),
                                 dcc.Dropdown(id='visualization-choice', clearable=False,
                                              options=[{'label': i.upper(), 'value': i} for i in
                                                       ['tsne', 'pca']],
                                              value='tsne'), ], width=2),
                        dbc.Button(id='load-dataset-button', children='Load',
                ...
                dbc.Container([
                    dbc.Row([
                        dbc.Col(
                            html.Div([
                                html.H3("Clustering", style={"text-align": "center"}),
                                dcc.Graph(id="clustering")
                            ]), width=8
                        ),
                        dbc.Col(
                            html.Div([
                                html.H3(id="selected-data-title", children="Selected Spectra",
                                        style={"text-align": "center"}),
                                dcc.Graph(id="selected-data-graph"),
                            ]), width=4)
                    ]),
                ], className="border border-secondary", fluid=True),
                ...
                dbc.Container([
                    dbc.Row([
                        dbc.Col([
                            DataTable(
                                id='table',
                                columns=[{"name": i, "id": i, "presentation": "markdown"} for i
                                         in ['sample', 'structure']],
                                data=[],
                                style_table={'overflow': 'auto', 'display': 'block',
                                             'height': '800px'}
                            ),
                        ], width=6),
                        dbc.Col([
                            html.Div([
                                ...
                                dcc.Dropdown(id='sample-name-dropdown-1', clearable=False,
                                             placeholder='loading...'),
                                dcc.RadioItems(
                                    id='add-trace-1-clustering',
                                    options=[{'label': i, 'value': i} for i in
                                             ['Display in clustering', 'Hide']],
                                    value='Hide'
                                ),
                                html.Div(id='trace-1', children=[
                                    dcc.RadioItems(
                                        id='display-structure-1-radio',
                                        options=[{'label': i, 'value': i} for i in
                                                 ['Display structure', 'Hide']],
                                        value='Display structure'
                                    ),
                                    html.Img(id="structure-1"),
                                ])
                            ])
                        ], width=3, className='border border-secondary'),
                        dbc.Col([
                            html.Div([
                                dcc.RadioItems(
                                ...
                                dcc.Dropdown(id='sample-name-dropdown-2', clearable=False,
                                             placeholder='loading...'),
                                dcc.RadioItems(
                ...
                dbc.Container([
                    dbc.Row([
                        dbc.Col([
                            html.Div([
                                html.H3("Spectra", style={'text-align': 'center'}),
                                dcc.Graph(id='spectra')
                            ])
                        ], width=12),
                    ])
                ], fluid=True, className='border border-secondary'),
                # Debug text area
                dbc.Container([
                    dbc.Row([
                        html.Div([
                            dcc.Textarea(id='debug-area',
                                         value='selectedData')
                        ], style={'display': 'none'}),
                    ])
                ], fluid=True),
    ...
    dbc.Tab(id='dendrogram-tab', label='Dendrogram', children=[
        dbc.Container([
            html.H1("COMING SOON!")
        ])
    ])
...
def display_clustering(collection, region, scaling, visualization):
    mapping = {'az': 'az-daughters',
               'mtp': 'mtp_reduced',
               'mtp and az': 'mtp-az',
               'fingerprint': 'finger',
               'normalized': 'normed'}
    dataset_clustering = '-'.join(
        [mapping[i] if i in mapping.keys() else i for i in
         [region, scaling, visualization, collection]]) + ".csv"
    ...
    if path_clustering.exists() and path_collection.exists():
        ...
        return df_clustering.to_json(orient='split'), df_collection.to_json(
            orient="split"), json.dumps(
            df_collection.index.to_list())
    else:
...
def update_spectrum(sample_name, sample_name_2, trace_1, trace_2,
                    dataset_collection_json):
    ...
    fig = go.Figure()
    if "D" in trace_1:
        fig.add_trace(go.Scatter(x=x, y=signal_1, mode='lines', name=sample_name,
                                 line_color=colors[0]))
    if "D" in trace_2:
        fig.add_trace(go.Scatter(x=x, y=signal_2, mode='lines', name=sample_name_2,
                                 line_color=colors[1]))
...
def update_clustering(sample_1, sample_2, trace_1, trace_2, data_as_json,
                      indices_as_json):
    ...
    fig.add_trace(
        go.Scatter(x=df.loc[truth_mtp, 'tsne_x'].to_list(),
    ...
    fig.add_trace(
        go.Scatter(x=df.loc[truth_az, 'tsne_x'].to_list(),
    ...
    fig.update_traces(marker_size=8, marker_line_color="black", marker_line_width=1,
...
def display_png(sample_name_1, sample_name_2, struct_1, struct_2):
    ...
    image_1_data = get_png_data(
        sample_name_1) if "D" in struct_1 else default_structure_image
    image_2_data = get_png_data(
        sample_name_2) if "D" in struct_2 else default_structure_image
...
def table_display(selected_data):
    if selected_data:
        ...
        for s in sample_names:
            ...
            chemical_name = smiles.loc[
                s, "Trivial name"] if s in smiles.index else "None"
            row = {'sample': s + ": " + chemical_name, 'structure': image_url}
            table_data.append(row)
        # each row is a dict
        style_data = [{'if': {'row_index': c[0], 'column_id': 'structure'},
                       'border': '2px solid ' + c[1]} for c in
                      enumerate(colors)]
    else:
